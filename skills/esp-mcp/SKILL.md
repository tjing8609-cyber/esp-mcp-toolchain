...
- Keep the first attachment review bounded to the base facts needed for safe development. Do not exhaustively archive a large schematic set in one pass.
- Before answering a later hardware question or operating the board, read `esp://hardwork/mapping`. If the task reveals a missing stable fact or stronger evidence, call `hardwork_mapping_patch` before finishing the answer or operation.
- Incremental mapping updates must preserve unrelated facts. Report mapping conflicts instead of silently replacing them.
...
2. If hardware documents are attached, archive and review them, then commit a bounded base GPIO and serial mapping.
...
7. Persist newly discovered hardware facts with `hardwork_mapping_patch`, including schematic or board-test evidence.
8. Parse errors and store stable learnings in project memory.
