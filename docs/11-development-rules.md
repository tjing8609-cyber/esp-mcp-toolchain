...
- 用户提供的主机相对路径必须以当前 `workspace_root` 为基准；不得依赖 MCP 进程当前目录、插件源目录或版本化安装缓存。主机绝对路径也必须位于当前 workspace 内，越界输入要在读取、建目录、写入或调用后端前拒绝。
- 板端 `remote_path` 与主机本地路径是不同命名空间，不得把工作区路径规则错误套到 MicroPython 文件系统。
- 主机路径回归测试必须把进程当前目录切换到无关目录，并同时断言项目内目标、项目外无副作用、后端未被越界输入调用及成功结果返回规范绝对路径。
...
- 烧录前必须对“最终实际将写入”的 bootloader、partition、app 等全部分段重新计算
  大小和 SHA-256。若 build 或 flash 子命令触发重编并导致任一已审查分段变化，必须
  立即停止后续串口/功能验收；烧录已经开始时，只有当前精确用户授权已经明确包含恢复，
  才能使用当次新鲜完整备份恢复。授权未包含恢复时必须停止、保存证据并请求确认，不得
  用烧录前旧哈希冒充实际写入输入。
