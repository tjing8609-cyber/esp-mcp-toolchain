...
7. SQLite schema 与仓储层：SQLite 已成为 runs/events 正式查询源；project-scoped schema、v1/JSONL 迁移、首次并发建库、事务序号、UUID 幂等、run 状态机、同步工具和异步 Monitor 生命周期候选实现已接通，本地跨工作树门禁已通过；提交、远端 CI 和插件同步待完成。
8. 日志查询增强：`run_id`、phase、level、tool、source、时间和 sequence 过滤候选实现已接通，并同步 CLI、FastMCP schema 与静态工具注册资源；本地门禁已通过，发布步骤待完成。后续可继续做导出、聚合和 run_id 前缀等非阻断增强。
9. 项目数据迁移体系：工程路径重绑定、项目合并、导出、导入和完整性校验。当前排在 SQLite 发布验证之后；它与数据库 schema 迁移是两类任务。

当前第 7 项和第 8 项的基础合同与本地 `134 passed` 门禁已完成。下一步是提交、远端 CI、当前项目日志迁移和插件源同步；不自动开始第 9 项。