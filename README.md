...
当前 SQLite 边界：正式项目数据库和当前安装插件仍保持 schema v2；v3-B4.2/B4.3 只生成文件证据候选，不判断正式数据库资格、不调用 B4.1，也不写 sidecar/marker。持 lease 的项目级扫描、raw 唯一归属、native profile 复核和实际补投影仍属于 B4.4；没有升级正式数据库、访问 COM3 或操作板卡
