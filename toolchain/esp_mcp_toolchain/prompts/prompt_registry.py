...
PROMPT_DEFINITIONS = {
    ...
    "performance_analysis": {
        ...
        "text": _workflow(
            ...
            success_evidence="samples 数量等于 iterations 且全部 ok；timing_us 和 memory_delta_bytes 给出 min/median/mean/max。samples、统计值和 sampling_profiler 状态写入当前 run 的 SQLite completion 事件，可按 run_id 复核。",
            safety_boundary="这是 instrumented_wall_time，不是采样 profiler；最多 50 次。每次都会执行目标并可能重复驱动蜂鸣器、GPIO 或写文件；未确认时 confirm_repeated_execution 必须保持 False，且不用结果推断功耗或电流。completion 事件不持久化 stdout 或内联 code，单条异常最多保留 256 字符；截断不是秘密检测，目标代码不得把口令、令牌或其他凭据写入异常信息。",
